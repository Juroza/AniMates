...
class TestProject(unittest.TestCase):
    ...
    LOCAL_DEV_URL="http://localhost:7071/add-users-to-project"  
    ...
    def test_add_users_to_project(self):
        owner=User(id=None,username="MinMin",password="Japsterdam")
        ...
        project= Project("History Of Knowing Ball",owner.username,True,[member1.username,member2.username])
    ...
    enable_cross_partition_query=True
        ...
        member3=User(id=None,username="Vera",password="champgne")
        self.userRepository.deleteUser(member3)
        self.userRepository.registerUser(member3)
        collection=project.users+[member3.username]
        project.setInvitedUsers(collection)
        response= requests.post(self.TEST_URL,json={'users':[member3.username],"project-name":project.projectName})
        self.assertEqual(200,response.status_code)
        projectRead=self.projectRepository.getProjectByName(project.projectName)
        readUsers=set(projectRead.users)
        users2=set(project.users)

        self.assertEqual(readUsers,users2)
