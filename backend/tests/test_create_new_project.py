...
class TestProject(unittest.TestCase):
    LOCAL_DEV_URL="http://localhost:7071/create-new-project"  
    ...
    def test_valid_create_new_project(self):
        ...
        response= requests.post(self.TEST_URL,json=project.to_dict())
    ...
    def test_invalid_user_create_new_project(self):
        owner=User(id=None,username="MyMy",password="Japsterdam")
        self.userRepository.deleteUser(owner)
        self.userRepository.registerUser(owner)
        member1=User(id=None,username="Coco",password="massaStudio")
        self.userRepository.deleteUser(member1)
        self.userRepository.registerUser(member1)
        member2=User(id=None,username="Barry Fake",password="chudRage")
    ...
    enable_cross_partition_query=True
        ...
        self.assertEqual(200,response.status_code)
        self.assertEqual(response.json(),{"result" :False, "msg": "User does not exist" })
    def test_invalid_owner_user_create_new_project(self):
        owner=User(id=None,username="MyMy Fake",password="Japsterdam")
        member1=User(id=None,username="Coco",password="massaStudio")
        self.userRepository.deleteUser(member1)
        self.userRepository.registerUser(member1)
        member2=User(id=None,username="Maya",password="chudRage")
    ...
    enable_cross_partition_query=True
        ...
        self.assertEqual(len(query_result),0)
        self.assertEqual(200,response.status_code)
        self.assertEqual(response.json(),{"result" :False, "msg": "Owner does not exist" })
    ...
    enable_cross_partition_query=True
        ...
        response= requests.post(self.TEST_URL,json=project.to_dict())
        self.assertEqual(200,response.status_code)
        self.assertEqual(response.json(),{"result" :False, "msg": "Project Already Exists" })
