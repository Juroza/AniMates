...
class TestDeleteFrame(unittest.TestCase): 
    LOCAL_DEV_URL="http://localhost:7071/add-frame-to-project"  
    TEST_URL = "http://localhost:7071/delete-frame"  
    ...
    "adaptiveStroke": true
        ...
        response= requests.post(self.TEST_URL,json={"name":frame.frameName})
        self.assertEqual(self.frameRepository.loadFrameByName(frame.frameName),None)
        print(response.json())
