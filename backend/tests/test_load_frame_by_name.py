...
class TestLoadFrameWithName(unittest.TestCase): 
    LOCAL_DEV_URL="http://localhost:7071/add-frame-to-project" 
    TEST_URL = "http://localhost:7071/load-frame" 
    ...
    "adaptiveStroke": true
        ...
        response= requests.get(self.TEST_URL,params={"name":frame.frameName})
        self.assertEqual(response.json(),frame.to_dict())
        print(response.json())
