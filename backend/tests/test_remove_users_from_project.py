...
class TestRemoveUserFromProject(unittest.TestCase):
    LOCAL_DEV_Create_URL="http://localhost:7071/create-new-project"  
    LOCAL_DEV_URL="http://localhost:7071/remove-users-from-project"  
    ...
    def test_remove_users_project(self):
        owner=User(id=None,username="Studio Massa",password="Japsterdam")
    ...
    enable_cross_partition_query=True
        ...
        response= requests.post(self.TEST_URL,json={'users':[member2.username],'project-name':project.projectName})
        self.assertEqual(200,response.status_code)
        projectUpdated=self.projectRepository.getProjectByName(project.projectName)
        self.assertEqual(projectUpdated.users,[member1.username])
